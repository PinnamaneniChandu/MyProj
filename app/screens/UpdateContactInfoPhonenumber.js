...
class UpdateContactInfoPhonenumber extends React.Component {
    constructor(props){        
        super(props);
    ...
    onMouseEnterHandler(event, disabled){ 
        disabled = true 
                ...
                if (((this.state.PareaCode==='' || this.state.PareaCode===undefined) && (this.state.MareaCode==='' || this.state.MareaCode===undefined) && (this.state.PphoneNumber===undefined || this.state.PphoneNumber==='') && (this.state.MphoneNumber ===undefined || this.state.MphoneNumber==='' ))||(array.length<=0 && (this.state.PareaCode==='' && this.state.MareaCode==='' && this.state.PphoneNumber==='' && this.state.MphoneNumber===''))||(!array.length<=0)){   
                    disabled = true
                    redirectTo = 'UpdateContactInfoPhonenumber'
                } 
                else{
                    disabled= false 
                ...
                <div className="blockUnderline"></div>
                    ...
                    <ErrorMessage errors={this.state.errorMessages}/>
                        ...
                        <div className="row addIncomeFormDivStyle">
                            ...
                            <ButtonFunctionality ButtonName={lang.ButtonCancel} redirectTo="UpdateContactInformationPage" dstClassName="btn button-text buttonBack1"/>
                            </div>
                            ...
                            <ButtonFunctionality ButtonName={lang.ButtonSave} redirectTo="UpdateProcessingContactInformation" disabled={this.state.disabled} dstClassName="btn buttonBack1 marginLeftForButtons" data= {PhoneData}/>            
                            </div> 
                        </div>
                    </form>
            </div>
...
export default connect(mapStateToProps,{phoneNumberInformation,validatePhoneNumber,validateAreaCode,validatePhoneNumberMandatory,validateAreaCodeMandatory,addPageLoggingInfo1,setSessionTime,setRequestTime,setResponseTime,setStartSessionTime})(UpdateContactInfoPhonenumber);